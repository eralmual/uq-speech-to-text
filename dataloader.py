...
class Dataloader():
    ...
    def load_dataset_raw_uq(repo_id="saul1917/SpaTrans_UQ_Bench_raw", partition_type = "test", partition_id = "partition_0", audio_extension = ".wav"):
        ...
        try:
            ...
            print(f"Error extracting audios (they might've been extracted already): {e}")
    ...
    @staticmethod
    def load_uq_partitions(partition: str, start_part: int = 1, last_part: int = 10) -> list[Dataset]:
        data_folds = []
        for i in range(start_part, last_part):
            # Load the fine tunning and test data
            fold = Dataloader.load_dataset_raw_uq(partition_type = partition, partition_id = f"partition_{i}")
            # Merge
