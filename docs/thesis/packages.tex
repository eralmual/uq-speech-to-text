...
% Allow including PDFs up to v1.7 (e.g. the scanned certificate.pdf)
...
% CSV table support (moved from thesis.tex)
\usepackage{csvsimple}

% Silence the harmless "No datatool support for dialect" notice that
% glossaries + babel (english/spanish) emit through tracklang.
\usepackage{silence}
\WarningFilter{tracklang}{No}

% Acronyms / glossaries -- load after hyperref
% nonumberlist: list the acronym definitions only, not page back-references
% nomain: only the acronym list is used, so skip the (empty) main glossary
