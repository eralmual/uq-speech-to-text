...
from levenshtein_monte_carlo_dropout import ScaledMonteCarloDropout
...
def run_experiment(exp_name: str, gen_kwargs: dict, embedding_kwargs: dict,
    ...
    for id in tqdm(range(start_fold, end_fold), desc="Evaluating target models"):
        ...
        # Use the number of samples specified
        if(test_size > 0):
            raise NotImplementedError("Test size limit is not implemented yet. Please set test_size to -1 to use the full test set.")
        ...
        if (exp_type == ExperimentType.FDE):
            ...
            smcd_model = ScaledMonteCarloDropout(model_name, num_iterations=num_iterations, dropout_rate=dropout_rate, device=device)
            # Transcribe audios and get uncertainties
