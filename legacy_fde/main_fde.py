...
#run_experiment(f"mcdstd-{num_iterations}i", {}, {}, exp_type=ExperimentType.MCD, device=dev, num_iterations=num_iterations, test_size=test_size)


#run_experiment(f"ts-{temperature}t", {}, {}, exp_type=ExperimentType.TS, device=dev, temperature=temperature, test_size=test_size)

#run_experiment(f"smcd-{num_iterations}i-{dropout_rate}d", {}, {}, exp_type=ExperimentType.SMCD,
#               device=dev, num_iterations=num_iterations, dropout_rate=dropout_rate,
#               test_size=test_size)

run_experiment(ExperimentType.LMCD, DatasetType.CALIBRATION, k_folds=10, sample_size=-1,
               temperature=temperature, dropout_rate=dropout_rate, dropout_iterations=num_iterations,
               output_dir="results", device=dev)
